...
################################# RMAggNet
rm_aggnet = RMAggNet([x for x in range(10)], RMModel, m=4, r=1, learning_rate=1e-3, load_path="trained_models/rmaggnet_mnist")
...
logger.info("= RMAggNet =\nRejected | Incorrect")
rm_aggnet.aggnet_eval(rm_aggnet, list(zip(noise_images, noise_labels)), batch_size=batch_size, thresholds=[0.5], max_correction=3, logger=logger, ood=True)
...
logger.info("= RMAggNet =\nRejected | Incorrect")
rm_aggnet.aggnet_eval(rm_aggnet, fmnist_dataset, batch_size=batch_size, thresholds=[0.5], max_correction=3, logger=logger, ood=True)
...
for attack_name in adversarial_attacks:
    ...
    for eps, adv_data in zip(epsilons, clipped_adv_eps):
        ...
        logger.info("- eps: {} -".format(eps))
        rm_aggnet.aggnet_eval(rm_aggnet, adv_dataset, batch_size=batch_size, thresholds=[0.5], max_correction=3, logger=logger)
