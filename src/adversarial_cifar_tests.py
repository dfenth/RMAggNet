...
logging.basicConfig(level=logging.INFO, filename="{}_adv_cifar10.log".format(logname))
...
################################# RMAggNet
rm_aggnet = RMAggNet([x for x in range(10)], RMModel, m=4, r=1, learning_rate=1e-3, load_path="trained_models/rmaggnet_cifar")


################################# Ensemble
ensemble_model = ensemble.Ensemble(EnsModel, 16, load_path="trained_models/ens_cifar")
...
ccat.ccat_model.load_state_dict(torch.load("trained_models/ccat_cifar.pth.tar"))
...
rm_aggnet.aggnet_eval(rm_aggnet, test_dataset, batch_size=batch_size, thresholds=[0.5], max_correction=3, logger=logger)
...
noise_images = torch.rand(size=(10000, 3, 32, 32))
...
base_model.load_state_dict(torch.load("trained_models/standard_cifar.pth.tar"))
base_model.to('cuda')
base_model.evaluate(test_loader, logger=logger)


adversarial_attacks = {
        "PGD (Linf)": [0, 0.05, 0.1, 0.3, 0.6, 1.0], 
        "PGD (L2)": [0, 0.3, 0.5, 0.75, 1, 2.5, 3], 
        "C&W L2": [0, 0.5, 1, 1.5, 2, 2.5, 3], 
        "Boundary (L2)": [1.5, 2, 2.5, 3],
}
...
for attack_name in adversarial_attacks:
    ...
    for idx, ep in enumerate(epsilons):
        save_image_sample(clipped_adv_eps[idx][:10].to('cpu'), "base_CIFAR_{}_{}.png".format(attack_name, str(ep).replace(".","_")))
    ...
    if len(clipped_adv_eps) > 0:
        for idx, ep in enumerate(epsilons):
            save_image_sample(clipped_adv_eps[idx][:10].to('cpu'), "ccat_CIFAR_{}_{}.png".format(attack_name, str(ep).replace(".","_")))
    ...
    # Save adversarial image samples for each epsilon
    for idx, ep in enumerate(epsilons):
        save_image_sample(clipped_adv_eps[idx][:10].to('cpu'), "ens_CIFAR_{}_{}.png".format(attack_name, str(ep).replace(".","_")))
    ...
    # Save adversarial image samples for each epsilon
    for idx, ep in enumerate(epsilons):
        save_image_sample(clipped_adv_eps[idx][:10].to('cpu'), "hybrid_CIFAR_{}_{}.png".format(attack_name.replace(" ", ""), str(ep).replace(".","_")))
