...
def attack_cifar(models, load_dir, attacks, attack_type, cuda):
    """
    ...
    if 'rmaggnet' in models:
        rm_aggnet = RMAggNet([x for x in range(10)], RMModel, m=4, r=1, load_path="{}/rmaggnet_cifar".format(load_dir), cuda=cuda)
    ...
    if 'ensemble' in models:
        ensemble_model = ensemble.Ensemble(EnsModel, 16, load_path="{}/ens_cifar".format(load_dir))
    ...
    if 'ccat' in models:
        ...
        ccat.ccat_model.load_state_dict(torch.load("{}/ccat_cifar.pth.tar".format(load_dir)))
    ...
    base_model.load_state_dict(torch.load("{}/standard_cifar.pth.tar".format(load_dir)))
    ...
    for attack_name in adversarial_attacks:
        ...
        if 'closedbox' in attack_type:
            ...
            for eps, adv_data in zip(epsilons, clipped_adv_eps):
                ...
                base_model.evaluate(adv_loader, logger=logger, cuda=cuda)
        ...
        if 'openbox' in attack_type:
            ...
            if 'rmaggnet' in models:
                ...
                for eps, adv_data in zip(epsilons, clipped_adv_eps):
                    ...
                    logger.info("== RMAggDiff ==")
                    logger.info("-- eps: {} --".format(eps))
                    logger.info(hybrid.evaluate(adv_loader, cuda=cuda))
